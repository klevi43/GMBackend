package org.kylecodes.gm.constants;

public final class NotFoundMsg {

    private NotFoundMsg() {
    }
    public static final String USER_NOT_FOUND_MSG = "No user found with given ID.";
    public static final String EMAIL_NOT_FOUND_MSG = "No such email found.";
    public static final String WORKOUT_NOT_FOUND_MSG = "No workout found with given id.";
    public static final String EXERCISE_NOT_FOUND_MSG = "No exercise found with given id.";
    public static final String SET_NOT_FOUND_MSG = "No set found with given id.";
}
