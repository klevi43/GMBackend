...
public interface ExerciseStatRepository extends JpaRepository<Set, Long> {
    ...
    @Query("""
    SELECT new org.kylecodes.gm.dtos.ExerciseDataDto(s.exercise.id, s.exercise.name, s.exercise.workout.name, s.weight, s.reps, s.exercise.workout.date)
    FROM set_tbl s WHERE s.exercise.workout.user.id = :userId
        AND s.exercise.name = :exerciseName
        AND s.exercise.workout.date BETWEEN :startDate AND :endDate
        AND s.weight = (
